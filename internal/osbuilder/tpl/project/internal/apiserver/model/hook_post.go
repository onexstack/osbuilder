...
// AfterCreate generates a {{.Web.R.Last.SingularLowerFirst}}ID after creating a database record.
func (m *{{.Web.R.GORMModel}}) AfterCreate(tx *gorm.DB) error {
	m.{{.Web.R.Last.SingularName}}ID = rid.NewResourceID("{{.Web.R.SingularLower}}").New(uint64(m.ID))
