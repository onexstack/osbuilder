...
const (
	newUsageStr = "cmd CMD_NAME | CMD_NAME CMD_DESCRIPTION"
)
...
This can improve your R&D efficiency.`)
	...
	newExample = templates.Examples(`
		...
		osbuilder cmd test -d /tmp/

		# Create a default 'test' command file with a description
		osbuilder cmd test "This is a test command"

		# Create command 'test' with two subcommands
		osbuilder cmd -g test "This is a test command with two subcommands"`)
...
// Use of this source code is governed by a MIT style
// license that can be found in the LICENSE file. The original repo for 
// this file is https://github.com/onexstack/osbuilder.
...
to quickly create a Cobra application.{{.Dot}})
	...
	{{.CommandName}}Example = templates.Examples({{.Dot}}
		...
		osbuilder {{.CommandName}} onexstack onexstackpass{{.Dot}})
...
to quickly create a Cobra application.{{.Dot}})
	...
	subcmd1Example = templates.Examples({{.Dot}}
		...
		osbuilder {{.CommandName}} subcmd1 onexstack onexstackpass

		# Print all option values for subcmd1 with --persistent specified
		osbuilder {{.CommandName}} subcmd1 onexstack onexstackpass --persistent="specified persistent option in command line"{{.Dot}})
...
to quickly create a Cobra application.{{.Dot}})
	...
	subcmd2Example = templates.Examples({{.Dot}}
		...
		osbuilder {{.CommandName}} subcmd2{{.Dot}})
)
...
// NewCmdCmd returns new initialized instance of 'new' sub command.
func NewCmdCmd(f cmdutil.Factory, ioStreams genericiooptions.IOStreams) *cobra.Command {
