...
type TagOptions struct {
	...
	PreReleaseMode string   // pre-release mode: "", "auto", "always"
...
var (
    ...
    Pre-release modes:
    - "" (none): Standard semantic versioning without pre-release handling
    - "auto": Automatically detect pre-release based on commit message tags
    - "always": Force all version increments to be pre-release versions

    Auto mode analyzes commit messages for pre-release indicators such as:
    - Commit scopes containing pre-release keywords (alpha, beta, rc, etc.)
    - Commit messages mentioning pre-release versions
    - Conventional commit patterns indicating experimental features

    Always mode is useful for development branches or when you want to ensure
    all releases are marked as pre-release until explicitly ready for production.
	...
	tagExample = templates.Examples(`# Tag the repository with the next calculated semantic version
        ...
        # Auto-detect pre-release based on commit messages
        osbuilder semver tag --prerelease-mode auto

        # Force all version increments to be pre-release
        osbuilder semver tag --prerelease-mode always

        # Use custom patch commit types with auto pre-release detection
        osbuilder semver tag --patch-types "fix,perf,security,refactor" --prerelease-mode auto

        # Complex configuration with metadata and always pre-release mode
        osbuilder semver tag --prerelease-mode always --metadata "ci.$(date +%Y%m%d).$(git rev-parse --short HEAD)" --patch-types "fix,perf,security"

        # Enable header trimming with auto pre-release detection
        osbuilder semver tag --trim-header --prerelease-mode auto
...
func NewTagCmd(factory cmdutil.Factory, ioStreams genericiooptions.IOStreams, semverOptions *SemverOptions) *cobra.Command {
	opts := &TagOptions{
		...
		PreReleaseMode: semver.PreReleaseModeNone, // Default to no pre-release handling
	}
	...
	cmd.Flags().StringVar(&opts.PreReleaseMode, "prerelease-mode", semver.PreReleaseModeNone, "Pre-release mode: 'auto' (detect from commits) or 'always' (force pre-release)")
	cmd.Flags().BoolVar(&opts.TrimHeader, "trim-header", false, "Trim header lines from commit messages during parsing")

	return cmd
}

// validatePreReleaseMode validates the pre-release mode value
func (o *TagOptions) validatePreReleaseMode(mode string) error {
	switch mode {
	case semver.PreReleaseModeNone, semver.PreReleaseModeAuto, semver.PreReleaseModeAlways:
		return nil
	default:
		return fmt.Errorf("invalid pre-release mode '%s': must be 'auto' or 'always'", string(mode))
	}
}
...
func (o *TagOptions) Complete() error {
	...
	// Validate pre-release mode
	if err := o.validatePreReleaseMode(o.PreReleaseMode); err != nil {
		return fmt.Errorf("invalid pre-release mode: %w", err)
	}

	// Auto-enable pre-release mode if explicit prerelease is specified
	if o.Prerelease != "" && o.PreReleaseMode == semver.PreReleaseModeNone {
		o.PreReleaseMode = semver.PreReleaseModeAuto
	}
...
func (o *TagOptions) Validate() error {
	...
	/*
		// Validate conflicting pre-release options
		if o.Prerelease != "" && o.PreReleaseMode == PreReleaseModeAlways {
			return fmt.Errorf("cannot specify explicit --prerelease when using --prerelease-mode=always")
		}
	*/
...
func (o *TagOptions) setupTagContext() (*context.Context, error) {
	...
	// Handle metadata and pre-release mode
	ctx.Metadata = o.Metadata
	ctx.PreReleaseMode = o.PreReleaseMode // Convert to string for context
